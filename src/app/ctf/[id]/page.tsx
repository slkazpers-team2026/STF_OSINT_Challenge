...
export default function CTFDetailPage({ params }: { params: { id: string } }) {
  ...
  // Writeup states
  const [activeTab, setActiveTab] = useState<'objectives' | 'writeups'>('objectives');
  ...
  // Writeups logic: Subscribe and Submit
  useEffect(() => {
    if (activeTab !== 'writeups' || !selectedChallengeId || !user) {
      setWriteups([]);
      return;
    }
    ...
    return () => unsubscribe();
  }, [activeTab, selectedChallengeId, user]);

  const handleWriteupSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !selectedChallengeId || !methodologyInput.trim()) return;
    ...
    try {
      const idToken = await user.getIdToken();
      const res = await submitWriteup(idToken, ctfId, selectedChallengeId, methodologyInput);
      if (res.success) {
        setWriteupMessage({ text: res.message, isError: false });
        setMethodologyInput('');
      } else {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl font-sans">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
          <div>
            <h1 className="text-4xl font-bold text-white flex items-center gap-3">
              ...
              {!ctfDetails.isPublished && (
                <span className="bg-yellow-950/30 text-yellow-500 text-base font-bold px-2.5 py-1 rounded border border-yellow-900 font-mono">
                  DRAFT
                </span>
              )}
            </h1>
            {(ctfDetails.creatorName || ctfDetails.displayName) && (
              <div className="text-base text-cyan-500/70 font-mono mt-1">
          ...
          <div className="flex flex-wrap gap-2">
            <a 
              ...
              className="bg-gray-700 hover:bg-gray-600 text-white font-medium py-2 px-4 rounded text-base transition-colors border border-gray-600 flex items-center justify-center"
            >
              View Leaderboard
            </a>
            <button
              ...
              className="text-base font-mono border border-red-500/30 text-red-400 bg-red-950/10 hover:bg-red-950/40 px-3 py-1.5 rounded transition-all disabled:opacity-50 flex items-center gap-1.5"
              ...
              <>
                {!ctfDetails.isPublished && (
                  <button
                    ...
                    className="bg-green-950/40 hover:bg-green-900/60 text-green-400 hover:text-green-300 font-bold py-2 px-4 rounded text-base transition-colors border border-green-800 flex items-center gap-1.5 font-mono"
                ...
                <button
                  ...
                  className="bg-cyan-950/40 hover:bg-cyan-900/60 text-cyan-400 hover:text-cyan-300 font-bold py-2 px-4 rounded text-base transition-colors border border-cyan-800 flex items-center gap-1.5 font-mono"
                ...
                <button
                  ...
                  className="bg-red-950/20 hover:bg-red-950/40 border border-red-900/60 hover:border-red-700 text-red-500 hover:text-red-400 font-bold py-2 px-4 rounded text-base transition-colors flex items-center gap-1.5 font-mono"
      ...
      {/* Tab Switcher */}
      <div className="flex items-center gap-6 border-b border-gray-700 mb-8 font-mono">
        <button
          onClick={() => setActiveTab('objectives')}
          className={`pb-3 text-lg font-bold tracking-wider transition-all relative ${
      ...
      {activeTab === 'objectives' ? (
          ...
          <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-2">
            ...
            {canDeploy && (
              <button
                ...
                className="bg-cyan-950/40 hover:bg-cyan-900/60 text-cyan-400 hover:text-cyan-300 font-bold py-1.5 px-4 rounded text-base transition-colors border border-cyan-800 flex items-center gap-1.5 font-mono"
                  ...
                  >
                    ...
                    {canDeploy && (
                      <div className="absolute top-4 right-4 flex gap-2 z-10">
                        <button
                          ...
                          className="text-sm px-2.5 py-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded text-cyan-400 hover:text-cyan-300 font-bold transition-colors"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDeleteChallenge(challenge.id)}
                          className="text-sm px-2.5 py-1 bg-red-950/20 hover:bg-red-950/40 border border-red-900 text-red-500 hover:text-red-400 font-bold rounded transition-colors"
                        >
                          Delete
                        </button>
                      </div>
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="mb-4">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span className="bg-cyan-950/30 text-cyan-500 text-sm font-bold px-2 py-0.5 rounded border border-cyan-900">
                            {challenge.points} PTS
                          </span>
                          {isSolved && (
                            <span className="bg-green-950/30 text-green-500 text-sm font-bold px-2 py-0.5 rounded border border-green-900 font-mono">
                              SOLVED
                            </span>
                          )}
                        </div>
                        {challenge.title && (
                          <div className="text-base font-bold text-white uppercase tracking-wider mt-1 border-l-2 border-cyan-800 pl-2">
                    ...
                    <div className="flex flex-col gap-2 mt-auto pt-4">
                      <input
                        ...
                        className="w-full px-4 py-2 bg-black border border-gray-800 rounded text-green-400 font-mono text-base focus:outline-none focus:border-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                      ...
                      <button
                        ...
                        disabled={!isUnlocked || isSolved}
                        className={`w-full font-bold py-2 px-4 rounded text-base transition-colors tracking-wider font-mono uppercase ${
                          isSolved
                    ...
                    {messages[challenge.id] && (
                      <p className={`mt-3 font-medium text-base ${messages[challenge.id].isError ? 'text-red-400' : 'text-green-400'}`}>
        ...
        <div className="font-mono">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {challenges.map((ch) => {
              const isCurrent = selectedChallengeId === ch.id;
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded text-base font-bold tracking-wider font-mono uppercase transition-all border flex items-center gap-2 ${
                    isCurrent
                      ? 'bg-cyan-950/40 border-cyan-500 text-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.3)]'
                      : 'bg-black border-gray-800 text-gray-400 hover:text-white hover:border-gray-700'
                  }`}
                >
                  <span>LEVEL {ch.levelId}</span>
                  {isSolved ? (
          ...
          {/* Detailed Writeup View for Selected Level */}
          {(() => {
            const selectedChallenge = challenges.find(ch => ch.id === selectedChallengeId);
            const isSolved = selectedChallengeId ? solvedChallengeIds.includes(selectedChallengeId) : false;

            if (!selectedChallenge) {
              return <p className="text-gray-500 italic text-base">[SELECT_A_LEVEL_TO_VIEW_DEBRIEFING_REPORTS]</p>;
            }

            if (!isSolved) {
              return (
                <div className="bg-black border border-red-955 p-12 rounded-lg relative overflow-hidden text-center min-h-[300px] flex flex-col items-center justify-center">
                  <div className="absolute inset-0 bg-red-955/5 backdrop-blur-[6px] pointer-events-none"></div>
                  <div className="relative z-10 space-y-4 max-w-md">
                    <div className="text-4xl animate-bounce-subtle">🔒</div>
                    <h3 className="text-red-500 font-extrabold tracking-widest text-base uppercase">
                      [ACCESS DENIED: METHODOLOGY ENCRYPTED UNTIL OBJECTIVE IS COMPLETED]
                    </h3>
                    <p className="text-sm text-red-400/80">
                      DECRYPTION HINT: You must successfully resolve Level {selectedChallenge.levelId} and submit the correct flag signature to unlock this debriefing archive.
                    </p>
                  </div>
                </div>
              );
            ...
            return (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Submit Writeup Section */}
                <div className="lg:col-span-1">
                  <form onSubmit={handleWriteupSubmit} className="space-y-4 bg-gray-900 p-6 rounded border border-gray-805">
                    <h4 className="text-base font-bold text-cyan-400 tracking-widest uppercase border-b border-gray-800 pb-2 mb-3">
                      {"// SUBMIT_METHODOLOGY // LEVEL_"}{selectedChallenge.levelId}
                    </h4>
                    <div>
                      <label className="block text-sm uppercase text-cyan-600 mb-2 font-semibold">
                        INVESTIGATION PROCESS BRIEF
                      </label>
                      <textarea
                        rows={6}
                        required
                        ...
                        placeholder="Detail your tools, search queries, reverse verification steps, and logical deduction..."
                        className="w-full px-3 py-2 bg-black border border-cyan-950 rounded text-cyan-400 font-mono text-base focus:outline-none focus:border-cyan-500 placeholder:text-cyan-950/60"
                      />
                    </div>
                    {writeupMessage && (
                      <p className={`text-base font-bold ${writeupMessage.isError ? 'text-red-400' : 'text-green-400'}`}>
                        {writeupMessage.text}
                      </p>
                    )}
                    <button
                      type="submit"
                      disabled={submittingWriteup}
                      className="w-full bg-cyan-950/40 hover:bg-cyan-900/60 border border-cyan-800 hover:border-cyan-500 text-cyan-400 py-2 rounded text-base font-bold tracking-widest uppercase transition-colors disabled:opacity-50"
                    >
                      {submittingWriteup ? 'SUBMITTING...' : 'LOG_DEBRIEFING_REPORT // 📡'}
                    </button>
                  </form>
                </div>

                {/* List of Writeups Section */}
                <div className="lg:col-span-2 space-y-6">
                  <h4 className="text-base font-bold text-gray-400 tracking-widest uppercase border-b border-gray-800 pb-2">
                    {"// ARCHIVED_DEBRIEFINGS // LEVEL_"}{selectedChallenge.levelId}
                  </h4>
                  {writeups.length === 0 ? (
                    <p className="text-gray-505 italic text-base">[NO_DEBRIEFING_REPORTS_RECORDED_FOR_LEVEL]</p>
                  ) : (
                    <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                      {writeups.map((w) => (
                        <div key={w.id} className="bg-gray-900/40 border border-gray-800/80 p-5 rounded relative hover:border-cyan-950/60 transition-all duration-300">
                          ...
                          <p className="text-gray-300 text-base whitespace-pre-wrap leading-relaxed font-sans">{w.methodology}</p>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          ...
          <div className="relative w-full max-w-lg p-6 bg-gray-900 border border-cyan-800 rounded shadow-2xl z-10 font-mono text-cyan-400 animate-fade-in">
            ...
            <div className="mb-6 border-b border-cyan-900/60 pb-3">
              ...
              <p className="text-sm text-cyan-700 mt-1">Modify Operation core parameters.</p>
            </div>
            <form onSubmit={handleUpdateCtf} className="space-y-4 text-base text-white">
              <div>
              ...
              <button
                type="submit"
                className="w-full py-2.5 px-4 bg-cyan-950/40 hover:bg-cyan-900/60 border border-cyan-700 hover:border-cyan-500 text-cyan-400 hover:text-cyan-300 font-bold rounded transition-colors tracking-wide font-mono uppercase text-base"
              >
                UPDATE_BRIEFING
              </button>
            </form>
